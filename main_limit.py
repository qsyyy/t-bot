...
async def process_single_tweet(tweet, includes, context):
    ...
    try:
        ...
        # 统一限速点（关键修改）
        await rate_limiter.wait()  # 所有发送操作前统一等待

        # 媒体处理逻辑
        if media_urls:
            media_group = []
            for idx, media in enumerate(media_urls[:4]):
                try:
                    media_type = InputMediaVideo if media['type'] == 'video' else InputMediaPhoto
                    media_item = media_type(
                        media=media['url'],
                        caption=caption if idx == 0 else None,  # 仅第一个媒体附带文本
        ...
        await rate_limiter.wait()  # 错误消息单独限速
